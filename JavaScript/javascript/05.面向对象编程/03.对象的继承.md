传统上，JavaScript 语言的继承不通过 class，而是通过“原型对象”（prototype）实现。ES6 引入了 class 语法，基于 class 的继承。
## 1.原型对象概述
## 1.1构造函数的缺点
